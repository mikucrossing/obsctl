...
### MIDI + Bluetooth 対応ビルド

ネイティブMIDI入力と Bluetooth 同期を有効にするにはビルドタグ `midi_native,bluetooth_native` が必要です。

- 開発起動:
  ```bash
  wails dev -tags midi_native,bluetooth_native
  # または
  go run -tags=dev,midi_native,bluetooth_native .
  ```
- 配布ビルド:
  ```bash
  wails build -tags midi_native,bluetooth_native -platform darwin/universal
  wails build -tags midi_native,bluetooth_native -platform windows/amd64 -nsis
  ```

ビルドタグを付けない場合はスタブ実装となり、MIDI/Bluetooth は未対応として表示されます。
...
- GUI（midi_native + bluetooth_native）: macOS（Universal `.zip` に `.app` 同梱）・Windows（`.exe` を `.zip`）
...
1. 右ペイン「シーン 同期（Bluetooth）」を開き、`enabled` をON、`role` を `parent` または `child` に設定します。
2. 親機では「親機コード発行」でペアリングコードを出し、子機でコード入力→「コードで参加」を実行します。
3. 親機を「開始」すると、GUIの手動シーン切替とMIDI起点の切替が、時刻指定で子機へ同期送信されます。
4. macOS は親機のみ対応で、子機モードは無効表示になります。
